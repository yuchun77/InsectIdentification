...
def choose_files():  #選擇照片並展示
    ...
    selectFilePaths = filedialog.askopenfilenames(title='選擇圖片', initialdir=os.getcwd(), filetypes=[('image files', '*.png *.jpeg *.jpg')])  #獲得圖片路徑
    global obj_state
    obj_state.set_value("")
    if selectFilePaths == "":   #按了選擇圖案又不選
        ...
        #這時才出現上一張、下一張的按鈕
        button_prev.place(anchor=tk.W, height=30, width=65, x=800, y=560)
        button_prev.config(command=lambda:prev_img("preview", int(e_for_page.get())))
        button_next.place(anchor=tk.W, height=30, width=65, x=910, y=560)
        button_next.config(command=lambda:next_img("preview", int(e_for_page.get())))
          
        #出現並設定目前頁數
        page_entry.place(anchor=tk.W, height=25, width=25, x=875, y=560)
        e_for_page.set(1)
        
        #填入共幾張
        label_for_num_page.place(x=930, y=575)
...
def choose_file():
    selectFilePath = filedialog.askopenfilename(title='選擇圖片', initialdir=os.getcwd(), filetypes=[('image files', '*.png *.jpeg *.jpg')])  #獲得圖片路徑
    if selectFilePath == "":   #按了選擇圖案又不選
        ...
        button_for_step.config(text="第一步：原圖進行gamma校正", command=lambda:Threader(gamma, path_entry.get()), state=tk.NORMAL)
        button_for_step.place(anchor=tk.NW, height=35, width=260, x=490, y=10)   #放上分步辨識按鈕
        button_for_choose_file.place(anchor=tk.NW, height=35, width=100, x=380, y=10) #放上選擇一張圖片按鈕
        path_entry.place(anchor=tk.NW, height=35, width=350, x=20, y=10)
        button_for_rechoose.place(anchor=tk.NW, height=35, width=130, x=850, y=10)
...
def choose_region():
    selectFilePath = filedialog.askopenfilename(title='選擇區域影像', initialdir=step_region_dir, filetypes=[('image files', '*.png *.jpeg *.jpg')])  #獲得圖片路徑
    if selectFilePath != "":
        if take_dir_path(selectFilePath) == step_region_dir:
            Threader(clahe, selectFilePath)
        else:
            messagebox.showinfo("提示", "請選擇剛才切割出的區域影像")
...
def show_one_img_result(page):
    ...
    #填入辨識結果
    label_for_result_category.place(width=120, x=800, y=110)
    label_for_result_category.config(text=tb_cat, justify=tk.LEFT)
    label_for_result_num.place(x=920, y=110)
...
def recognize(file_paths):
    if file_paths == "":  #還沒選照片就按開始辨識
        ...
        #填入辨識結果
        label_for_result_title.config(text="此張照片辨識結果")
        label_for_result_title.place(x=820, y=80)
        img_open = Image.open('.\\img\\line.png')
        put_img_specify_width(img_open, 160, label_for_line)
        label_for_line.place(x=800, y=100)
        show_one_img_result(1)  #第1張照片的結果(從page1開始)
        
        #填入共幾張
        label_for_num_page.config(text="共"+str(len(big_img_list))+"張")

        #出現查看本次辨識結果按鈕
        button_for_allresult.place(anchor=tk.NW, height=35, width=180, x=580, y=10)
...
def loading_gif(obj_state):
    ...
    label_for_loading.place(x=455, y=240)
...
def gamma(file_path):
    if file_path == "":  #還沒選照片就按gamma
        ...
        img_width, img_height = put_step_imgandtext(frame_for_step_p1, 0, 0, img, 310, "原圖", 35)
        ...
        #更改步驟button
        button_for_step.config(text="第二步：YOLO定位偵測", command=lambda:Threader(yolo, img_gamma), state=tk.NORMAL)
...
def yolo(img_gamma):
    ...
    #填入YOLO辨識後的照片及說明文字
    put_step_imgandtext(frame_for_step_p1, 2, 0, img_draw, 310, "yolo辨識結果", 35)
    
    #更改步驟button
    button_for_step.config(text="第三步：座標切割原圖", command=lambda:Threader(cut, final_ratio_boxes), state=tk.NORMAL)
...
def cut(final_ratio_boxes):
    ...
    time = datetime.now().strftime('%Y%m%d%H%M%S')  #取得現在時間
    step_dir = r"./step/step_"+time+"/"  #結尾有沒有加\\都可以
    ...
    tk.Label(frame_for_step_p1, text="座標切割原圖", pady=8, font=fontStyle_normal_text).grid(row=3, column=2)

    #更改步驟button
    button_for_step.config(text="第四步：擇一區域影像進行clahe", command=choose_region, state=tk.NORMAL)
...
def clahe(img_path):
    ...
    put_step_imgandtext(frame_for_step_p2, 0, 0, img, 200, "區域影像", 20)
    ...
    #更改步驟button
    button_for_step.config(text="第五步：主要模型種類辨識", command=lambda:Threader(big_model_recognize, img_clahe, img), state=tk.NORMAL)
...
def big_model_recognize(img_clahe, img):
    ...
    #小model再次辨識或完成辨識
    if decoded_label == "蛾" or decoded_label == "麥蛾" or decoded_label == "螟蛾" or decoded_label == "蕈蛾":
        #更改步驟button
        button_for_step.config(text="第六步：ModelA次要模型種類辨識", command=lambda:Threader(little_model_recognize, "moth",img, predict_prob), state=tk.NORMAL)
    elif decoded_label == "小蜂" or decoded_label == "蟻" or decoded_label == "蠅" or decoded_label == "隱翅蟲":
        #更改步驟button
        button_for_step.config(text="第六步：ModelB次要模型種類辨識", command=lambda:Threader(little_model_recognize, "bee", img, predict_prob), state=tk.NORMAL)
    else:
        #更改步驟button
        button_for_step.config(text="完成", state=tk.DISABLED)
...
def little_model_recognize(little_model_name, img, predict_prob):
    ...
    #更改步驟button
    button_for_step.config(text="第七步：模型合併", command=lambda:Threader(merge_two_model, predict_prob, predict_prob_by_lmodel, little_model_name), state=tk.NORMAL)
...
def show(purpose):
    if purpose == "seperate":
        button_for_step.config(text="第一步：原圖進行gamma校正", command=lambda:Threader(gamma, path_entry.get()), state=tk.DISABLED)
        button_for_step.place(anchor=tk.NW, height=35, width=260, x=490, y=10)   #放上分步辨識按鈕
        button_for_choose_file.place(anchor=tk.NW, height=35, width=100, x=380, y=10) #放上選擇一張圖片按鈕
    elif purpose == "direct":
        button_for_recognize.config(state=tk.DISABLED)
        button_for_recognize.place(anchor=tk.NW, height=35, width=90, x=480, y=10)  #放上直接辨識按鈕
        button_for_choose_files.place(anchor=tk.NW, height=35, width=90, x=380, y=10) #放上選擇多張圖片按鈕
    path_entry.place(anchor=tk.NW, height=35, width=350, x=20, y=10)
    button_for_rechoose.place(anchor=tk.NW, height=35, width=130, x=850, y=10)
...
def rechoose():
    ...
    e_for_path.set("")
    #出現分步辨識、直接辨識的按鈕
    button_for_seperate.place(anchor=tk.NW, height=100, width=250, x=200, y=250)
    button_for_direct.place(anchor=tk.NW, height=100, width=250, x=550, y=250)
...
if __name__ == '__main__':
    ...
    ##最一開始的兩個按鈕
    #分步辨識
    button_for_seperate = ttk.Button(root, text="分步辨識", command=lambda:show("seperate"), style='start.TButton', takefocus=0)
    button_for_seperate.place(anchor=tk.NW, height=100, width=250, x=200, y=250)
    #直接辨識
    button_for_direct = ttk.Button(root, text="直接辨識", command=lambda:show("direct"), style='start.TButton', takefocus=0)
    button_for_direct.place(anchor=tk.NW, height=100, width=250, x=550, y=250)
    ...
    button_for_choose_file = ttk.Button(root, text="選擇一影像", command=choose_file, style='normalbutton.TButton', takefocus=0)
    button_for_step = ttk.Button(root, style='normalbutton.TButton', takefocus=0)
    button_for_rechoose = ttk.Button(root, text="重選展示方式", command=rechoose, style='normalbutton.TButton', takefocus=0)
    ...
    button_for_choose_files = ttk.Button(root, text="選擇影像", command=choose_files, style='normalbutton.TButton', takefocus=0)
    button_for_recognize = ttk.Button(root, text="開始辨識", command=lambda:recognize(path_entry.get()), style='normalbutton.TButton', takefocus=0, state=tk.DISABLED)
    button_for_allresult = ttk.Button(root, text="查看本次辨識結果", command=show_all_result, style='normalbutton.TButton', takefocus=0)
    ...
    #上一張、下一張 (先定義還沒擺上去)
    button_prev = ttk.Button(root, text="上一張", style='littlebutton.TButton', takefocus=0)
    button_next = ttk.Button(root, text="下一張", style='littlebutton.TButton', takefocus=0)
